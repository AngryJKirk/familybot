...
class TopPidorsByMonthsExecutor(val commonRepository: CommonRepository,
                                val pidorDictionaryRepository: PidorDictionaryRepository) : CommandExecutor() {
    ...
    override fun execute(update: Update): (AbsSender) -> Unit {
        val pidors = commonRepository.getPidorsByChat(update.toChat()).filter { it.date.isBefore(startOfMonth()) }
        val result = pidors.groupBy { map(it.date) }
