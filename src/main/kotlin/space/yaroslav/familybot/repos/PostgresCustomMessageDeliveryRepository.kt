...
class PostgresCustomMessageDeliveryRepository(val jdbcTemplate: JdbcTemplate) : CustomMessageDeliveryRepository {
    override fun hasNewMessages(chat: Chat): Boolean {
        return jdbcTemplate.query("SELECT 1 FROM custom_message_delivery where chat_id = ? and is_delivered is false", RowMapper { _, _ -> 1 }, chat.id).isNotEmpty()
    }

    override fun getNewMessages(chat: Chat): List<CustomMessage> {
        return jdbcTemplate.query("SELECT * FROM custom_message_delivery where chat_id = ? and is_delivered is false", RowMapper { rs, _ ->
            CustomMessage(
                    rs.getLong("id"),
